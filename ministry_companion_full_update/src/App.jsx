...
/**
 * Ministry Companion — App.jsx (patched full file)
 * 
 * Fixes & features implemented here:
 * - Header “Ministry Companion” + simple splash
 * - Home: daily thought overwrite, service hours (categories + edit), monthly export
 * - Territory: person name, edit saved visits (including ADDRESS with duplicate warning), timestamps
 *   NH export (today-only bullets) includes 🕒 time
 * - Return Visits: Suggest next scripture popup (pulls from Scripture Tool graph), edit; timestamps
 * - Bible Studies: “Lesson number” labeled; edit; timestamps
 * - Scripture Tool: predictive typeahead, closes on blur/submit, normalizes dashes; JSON import
 * 
 * Data is localStorage-only (key: mc_state_v1). Safe to replace file; your data remains.
 */
...
const TOPIC_LIBRARY_STARTER = [
  { topic: "Suffering & Loss", refs: ["Rev 21:3-4","Ps 34:18","2 Cor 1:3-4"],
    questions: ["How would life feel if tears and pain truly ended?","Which loss weighs on you most right now?"] },
  { topic: "God’s Name & Identity", refs: ["Ps 83:18","Ex 3:15","Matt 6:9"],
    questions: ["Have you ever seen God’s personal name in your Bible?","Why might God want us to know and use his name?"] }
];
const SEED = {
  territoryNumber: "", dailyThoughts: [], serviceLogs: [],
  households: [], visits: [], returnVisits: [], studies: [],
  scriptureSuggest: SCRIPTURE_SUGGEST_STARTER, topicLibrary: TOPIC_LIBRARY_STARTER
};

// ---------------- UI Primitives ----------------
const Button = ({ children, className = "", ...props }) => (
  <button
    className={"px-3 py-2 rounded-xl shadow-sm border border-neutral-300 dark:border-neutral-700 hover:shadow transition active:scale-[.98] "+className}
    {...props}
  >{children}</button>
);
const Input = (props)=>(
  <input {...props}
    className={(props.className||"")+" w-full px-3 py-2 rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900"}
  />
);
...
const TextArea = (props)=>(
  <textarea {...props}
    className={(props.className||"")+" w-full px-3 py-2 rounded-xl border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 min-h-[80px]"}
  />
);
...
const addDailyThought = (text, setState) => {
  if(!text?.trim()) return; const today = todayISO();
  setState(s => ({ ...s, dailyThoughts: [
...
const addDoorLog = ({address,status,scripture,literature,notes,personName}, state, setState) => {
  ...
  if(existingIdx !== -1){ if(!confirm(`A household for "${address}" already exists. Add another entry with the same address?`)) return; }
  ...
  const visit = { id: uid("V"), date: todayISO(), householdId: idx, personId: null, status, scripture, literature, notes, personName, createdAt: Date.now(), updatedAt: Date.now() };
  const visits = [...state.visits, visit];
  let returnVisits = state.returnVisits;
  if(status==="RV"){
    returnVisits = [ { id: uid("RV"), personName: personName||"(unknown)", address, lastScripture: scripture||"", nextScripture: "", nextVisit: todayISO(), bestTime: "", priority: "Medium", notes: notes||"", createdAt: Date.now(), updatedAt: Date.now() }, ...returnVisits ];
  }
...
function PageHome({ state, setState }) {
  ...
  const [hours, setHours] = useState(""); const [cat, setCat] = useState("Door-to-door"); const [hNotes, setHNotes] = useState("");
  ...
  const addHours = () => {
    if(!hours) return;
    setState(s => ({ ...s, serviceLogs: [...(s.serviceLogs||[]), { id: uid("HRS"), date: todayISO(), hours: Number(hours), category: cat, notes: hNotes, createdAt: Date.now(), updatedAt: Date.now() } ] }));
    setHours(""); setHNotes("");
  };
  const exportMonth = () => {
    const lines = [
  ...
  return (<div className="p-1">
    <Section title="Territory & Links" right={
    ...
    <Section title="Daily Thought from Conductor" right={<Button onClick={()=>{ addDailyThought(thought,setState); setThought(""); }}>Save</Button>}>
      ...
      {todays.length>0 && (<div className="mt-3 text-sm opacity-80"><div className="font-medium mb-1">Today’s note</div><ul className="list-disc ml-6">{todays.map(t=>(<li key={t.id}>{t.text} <span className="opacity-60">• saved {prettyTime(t.createdAt)}</span></li>))}</ul></div>)}
    </Section>
    ...
    <Modal open={!!edit} onClose={()=>setEdit(null)} title="Edit Hours">
      {edit && (<div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Button className="bg-blue-600 text-white" onClick={()=>{ setState(s=>({...s, serviceLogs: s.serviceLogs.map(x=> x.id===edit.id ? {...edit, updatedAt: Date.now()} : x)})); setEdit(null); }}>Save</Button>
...
function PageTerritory({ state, setState }) {
  ...
  const [scripture,setScripture]=useState(""); const [literature,setLiterature]=useState(""); const [notes,setNotes]=useState("");
    ...
    setState(s=>{
      ...
      if (households[hid]) {
        ...
        if (newAddr) {
          ...
          if (dupAt !== -1) { if(!confirm(`Another household already uses “${newAddr}”. Keep both with the same address?`)) return s; }
  ...
  return (<div>
    ...
    <Section title="Export NH / Follow-up List" right={<div className="flex items-center gap-2">
      <label className="text-sm flex items-center gap-2"><input type="checkbox" checked={nhOnly} onChange={e=>setNhOnly(e.target.checked)} /> NH only</label>
      ...
      <div className="space-y-2">
        {recent.map(v => (
          <div key={v.id} className="rounded-xl border p-3 border-neutral-200 dark:border-neutral-800">
            ...
            <div className="font-medium">{state.households[v.householdId]?.address || "(unknown)"} — <span className="px-2 py-0.5 rounded-full text-xs border">{v.status}</span></div>
    ...
    <Modal open={!!editing} onClose={()=>setEditing(null)} title="Edit Visit">
      {editing && (<div className="space-y-2">
        <Input placeholder="Address" value={(editing.__address ?? state.households[editing.householdId]?.address) || ""} onChange={e=>setEditing({...editing, __address: e.target.value})} />
...
function PageReturnVisits({ state, setState }) {
  const [form,setForm] = useState({ personName:"", address:"", lastScripture:"", nextScripture:"", nextVisit: todayISO(), bestTime:"", notes:"" });
  ...
  return (<div>
      ...
      <div className="space-y-2">
        {sorted.map(rv => (
          <div key={rv.id} className="rounded-xl border p-3 border-neutral-200 dark:border-neutral-800">
            <div className="flex items-center justify-between">
              <div className="font-semibold">{rv.personName} — <span className="opacity-70 font-normal">{rv.address}</span></div>
              <div className="flex gap-2">
    ...
    <Modal open={!!editing} onClose={()=>setEditing(null)} title="Edit Return Visit">
      {editing && (<div className="space-y-2">
        ...
        <TextArea placeholder="Notes" value={editing.notes||""} onChange={e=>setEditing({...editing, notes:e.target.value})} />
        <div className="flex gap-2">
          <Button className="bg-blue-600 text-white" onClick={()=>{ setState(s=>({...s, returnVisits: s.returnVisits.map(x=> x.id===editing.id ? {...editing, updatedAt: Date.now()} : x)})); setEditing(null);} }>Save</Button>
          <Button onClick={()=>setEditing(null)}>Cancel</Button>
        </div>
      </div>)}
    </Modal>
    ...
    <Modal open={!!suggestFor} onClose={()=>setSuggestFor(null)} title="Suggested next scriptures">
      {suggestFor && (<div className="space-y-2">
        {(() => {
          const rv = suggestFor.kind === "rv" ? suggestFor.data : suggestFor.data;
          const list = suggestions(rv);
          if (list.length === 0) {
            return <div className="opacity-70">No suggestions found for <b>{rv.lastScripture || "(none)"} </b>. Try “Ps 83:18” vs “Psalm 83:18”, or import more mappings in the Scripture Tool.</div>;
          }
          return (<ul className="space-y-2">
            {list.map((s,i)=>(
              <li key={i} className="border rounded-xl p-3 border-neutral-200 dark:border-neutral-800 flex items-center justify-between">
                <div><div className="font-medium">{s.ref}</div><div className="text-sm opacity-80">{s.why}</div>{s.ask && <div className="text-sm mt-1">Q: {s.ask}</div>}</div>
                <Button onClick={()=>{
                  if(suggestFor.kind==="rv"){ const id = suggestFor.data.id; setState(st=>({...st, returnVisits: st.returnVisits.map(x=> x.id===id ? {...x, nextScripture: s.ref, updatedAt: Date.now()} : x)})); }
                  else { setForm(f=>({...f, nextScripture: s.ref})); }
                  setSuggestFor(null);
                }}>Use</Button>
              </li>
            ))}
...
function PageStudies({ state, setState }) {
  const [name,setName]=useState(""); const [lesson,setLesson]=useState(1); const [nextDate,setNextDate]=useState(""); const [notes,setNotes]=useState(""); const [editing,setEditing]=useState(null);
  const add = () => {
    if(!name) return;
    setState(s=>({...s, studies:[ { id: uid("S"), name, lesson: Number(lesson)||1, nextDate, notes, createdAt: Date.now(), updatedAt: Date.now() }, ...s.studies ] }));
    setName(""); setLesson(1); setNextDate(""); setNotes("");
  };
  const markDone = (id)=> setState(st=>({...st, studies: st.studies.map(x=> x.id===id ? {...x, lesson: Number(x.lesson)+1, updatedAt: Date.now()} : x) }));
  return (<div>
        ...
        {state.studies.map(s => (
          <div key={s.id} className="rounded-xl border p-3 border-neutral-200 dark:border-neutral-800">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2"><div className="text-sm opacity-75">Lesson {s.lesson}</div><Button className="text-xs" onClick={()=>setEditing(s)}>Edit</Button></div>
            </div>
    ...
    <Modal open={!!editing} onClose={()=>setEditing(null)} title="Edit Study">
      {editing && (<div className="space-y-2">
        ...
        <div><div className="text-xs mb-1 opacity-70">Lesson number</div><Input type="number" min={1} max={60} value={editing.lesson} onChange={e=>setEditing({...editing, lesson: Number(e.target.value)})} /></div>
        <Input type="date" value={editing.nextDate || ""} onChange={e=>setEditing({...editing, nextDate: e.target.value})} />
        <TextArea placeholder="Notes" value={editing.notes || ""} onChange={e=>setEditing({...editing, notes: e.target.value})} />
        <div className="flex gap-2"><Button className="bg-blue-600 text-white" onClick={()=>{ setState(s=>({...s, studies: s.studies.map(x=> x.id===editing.id ? {...editing, updatedAt: Date.now()} : x)})); setEditing(null); }}>Save</Button><Button onClick={()=>setEditing(null)}>Cancel</Button></div>
      </div>)}
    </Modal>
  </div>);
}
...
function PageScriptureTool({ state, setState }) {
  const MAX_INLINE = 5, TYPEAHEAD_CAP = 12, MIN_CHARS = 2;
  ...
  const [submitted,setSubmitted]=useState(false); const [focus,setFocus]=useState(false);
  ...
  const keywordOptions = useMemo(() => {
    ...
    const n = dq.toLowerCase(); if(!n || !focus) return [];
    return all.filter(x => x.toLowerCase().includes(n)).slice(0, TYPEAHEAD_CAP);
  }, [dq, state.topicLibrary, graph, focus]);

  const topics = useMemo(()=>{
    const lib = state.topicLibrary || []; if(!submitted || dq.trim().length<MIN_CHARS) return [];
    const needle = dq.trim(); const nNeedle = normRef(needle); const isScriptureLike = /\d?\s?[a-z]+\s?\d+(?::\d+)?/i.test(needle);
    return lib.filter(t => {
      const titleHit = t.topic.toLowerCase().includes(needle.toLowerCase()); if(titleHit) return true;
      if(!isScriptureLike) return false; const refs = Array.isArray(t.refs)?t.refs:[];
      return refs.some(r => { const nr = normRef(r); return nr===nNeedle || nr.startsWith(nNeedle); });
    });
  },[submitted,dq,state.topicLibrary]);

  const [jsonText,setJsonText]=useState("");
  const importGraph = () => {
    try { const data = JSON.parse(jsonText); const sug = data.scriptureSuggest || data; const lib = data.topicLibrary || state.topicLibrary;
      setState(s=>({...s, scriptureSuggest: sug, topicLibrary: lib })); setJsonText(""); alert("Imported. Suggestions updated.");
    } catch { alert("Invalid JSON. Expect {scriptureSuggest:{}, topicLibrary: []}"); }
  };
  useEffect(()=>{
    const onDoc = (e) => { const c = document.getElementById("topic-search-container"); if(c && !c.contains(e.target)) setFocus(false); };
  ...
  return (<div>
    <Section title="Auto-Suggest (based on last scripture)">
      <div className="grid md:grid-cols-3 gap-3 items-start">
        ...
        <div className="md:col-span-2 space-y-2">
          {inlineSuggestions.length===0 ? (<div className="opacity-70">No suggestions found for “{lastShared}”. Try Ps ↔ Psalm, or import a larger graph below.</div>)
          : (<ul className="list-disc ml-5">{inlineSuggestions.map((s,i)=>(<li key={i}><span className="font-medium">{s.ref}</span> — {s.why}{s.ask ? ` — Q: ${s.ask}` : ""}</li>))}</ul>)}
    ...
    <Section title="Search by Concern or Topic">
      <form id="topic-search-container" onSubmit={(e)=>{ e.preventDefault(); setSubmitted(true); setFocus(false); }} className="relative">
        <Input onFocus={()=>setFocus(true)} placeholder="Type a keyword (e.g., anxiety, hope)… or a scripture (e.g., Rev 21:4); then press Enter" value={q} onChange={e=>{ setQ(e.target.value); setSubmitted(false); }} />
        {keywordOptions.length>0 && (<div className="absolute z-10 mt-1 w-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow max-h-64 overflow-auto">
          {keywordOptions.map(opt => (<div key={opt} className="px-3 py-2 cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-800" onClick={()=>{ setQ(opt); setSubmitted(true); setFocus(false); }}>{opt}</div>))}
        </div>)}
      ...
      {submitted && q.trim().length>=2 && (<div className="grid md:grid-cols-2 gap-3 mt-3">
        {topics.length===0 ? <div className="opacity-70">No topics found for “{q}”.</div>
        : topics.map(t => (<div key={t.topic} className="rounded-xl border p-3 border-neutral-200 dark:border-neutral-800">
            ...
            {Array.isArray(t.questions) && t.questions.length>0 && (<div className="text-sm mt-1"><span className="font-medium">Sample questions:</span><ul className="list-disc ml-5">{t.questions.map((qq,i)=>(<li key={i}>{qq}</li>))}</ul></div>)}
          </div>))
        }
      </div>)}
    </Section>
    ...
    <Section title="Import Recommendations (JSON)">
      <div className="text-sm opacity-75 mb-2">Paste a JSON object with <code>scriptureSuggest</code> and/or <code>topicLibrary</code>.</div>
      <TextArea placeholder='{"scriptureSuggest":{"Ps 83:18":[{"ref":"Rev 21:3-4","why":"..."}]},"topicLibrary":[{"topic":"...","refs":["..."],"questions":["..."]}] }' value={jsonText} onChange={e=>setJsonText(e.target.value)} />
...
function Stats({ state }){
  const today = todayISO(); const todays = (state.visits||[]).filter(v=>v.date===today);
...
const StatCard = ({label, value}) => (<div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 p-4 text-center bg-white/70 dark:bg-neutral-900">
  <div className="text-2xl font-bold">{value}</div><div className="text-sm opacity-70">{label}</div>
</div>);
...
export default function MinistryCompanion(){
  ...
  return (<div className="min-h-[100vh] bg-neutral-100 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100">
    {showSplash && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-b from-amber-100 to-white dark:from-neutral-800 dark:to-black">
      <div className="text-center"><div className="text-3xl font-bold mb-2">Ministry Companion</div><div className="text-xs opacity-70">Loading…</div>
        <button className="mt-3 text-xs underline" onClick={()=>setShowSplash(false)}>Skip</button>
      </div>
    </div>)}
    ...
    <div className="max-w-5xl mx-auto">
      <nav className="sticky top-0 z-10 backdrop-blur bg-neutral-100/70 dark:bg-neutral-950/70 border-b border-neutral-200/60 dark:border-neutral-800">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="text-xl font-bold">Ministry Companion</div>
          <div className="flex gap-1 overflow-x-auto">
            {TABS.map(t => (<Button key={t} onClick={()=>setTab(t)} className={`text-sm ${tab===t ? "bg-neutral-900 text-white dark:bg-white dark:text-neutral-900":"bg-neutral-50 dark:bg-neutral-800"}`}>{t}</Button>))}
          </div>
        </div>
      </nav>
      ...
      <main className="px-4 py-4">
        <Stats state={state} />
        <div className="mt-4" />
        {tab==="Home" && <PageHome state={state} setState={setState} />}
